...
actor Sovereign {
  ...
  let DEFAULT_NODE_COUNT : Nat = 4000;
  ...
  let nodes      = Buffer.Buffer<Node>(2048);
  let canisters  = Buffer.Buffer<SovereignCanister>(64);
  let chain      = Buffer.Buffer<SovereignBlock>(256);
  let rounds     = Buffer.Buffer<ConsensusRound>(128);
  let identities = Buffer.Buffer<GoldenIdentity>(2048);
  let substrateLog = Buffer.Buffer<Text>(512);
  ...
  let GOLDEN_THRESHOLD : Float = PHI / (PHI + 1.0); // ≈ 0.618

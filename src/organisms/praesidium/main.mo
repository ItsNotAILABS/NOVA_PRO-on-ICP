...
actor Praesidium {
  ...
  let SAFE_THRESHOLD : Float    = 0.618;   // φ/(φ+1) — below this, route is safe
  let ELEVATED_THRESHOLD : Float = 0.382;  // 1/φ²
  let CRITICAL_THRESHOLD : Float = 0.236;  // 1/φ³
  let ARMOR_LAYERS : Nat = 7;              // Per AEGIS spec
  ...
  let endpoints   = Buffer.Buffer<EndpointRecord>(64);
  let defenseEvents = Buffer.Buffer<DefenseEvent>(256);
  let honeypots   = Buffer.Buffer<HoneypotField>(16);
  let crusaders   = Buffer.Buffer<CrusaderUnit>(32);
  let defenseLog  = Buffer.Buffer<Text>(256);

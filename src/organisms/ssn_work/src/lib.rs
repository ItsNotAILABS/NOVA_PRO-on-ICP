...
/// Burn effect: Increases SSN reputation (via cross-canister call)
///
...
const TOKEN_NAME: &str = "SSN-WORK";
const TOKEN_SYMBOL: &str = "SSNW";
const TOKEN_DECIMALS: u8 = 8;
const TRANSFER_FEE: u128 = 0; // No fee on sub-coin transfers within organism
const MINT_CAP_SCALE: u128 = 1_000_000;
...
struct TokenState {
    /// Balance ledger: Account -> amount (in e8s)
    balances: HashMap<String, u128>,
    /// Total supply
    total_supply: u128,
    /// Minting authority (ssn_token canister principal)
    minting_authority: Option<Principal>,
    /// SSN-ID binding: tracks which SSN minted tokens to which account
    ...
    /// Frozen accounts (associated with banned SSNs)
...
// Simple hex encoding (avoid external dep)
mod hex {
...
/// φ-based mint cap: floor(stake^(1/φ) × reputation × SCALE)
...
#[post_upgrade]
fn post_upgrade() {
    // State restored from stable memory in production
}
...
fn icrc1_metadata() -> Vec<(String, MetadataValue)> {
    vec![
        ...
        ("ssn:type".to_string(), MetadataValue::Text("contribution_labor".to_string())),
    ]
}
...
fn icrc1_transfer(arg: TransferArg) -> Result<Nat, TransferError> {
    ...
    STATE.with(|s| {
        ...
        // Fee is burned (deflationary) — but fee is 0 for sub-coins
        if TRANSFER_FEE > 0 {
...
// ═══════════════════════════════════════════════════════════════════
//  BURNING (increases SSN reputation via ssn_token canister)
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════
//  SSN GOVERNANCE (Freeze/Unfreeze accounts)
// ═══════════════════════════════════════════════════════════════════
...
fn health() -> String {
    STATE.with(|s| {
        let state = s.borrow();
        format!(
            "{} | Supply: {} | Accounts: {} | Txns: {} | φ-powered",
            TOKEN_NAME,
            state.total_supply,
            state.balances.len(),
            state.tx_index
